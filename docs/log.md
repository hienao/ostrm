...
1. 登录管理后台
2. 点击左侧菜单「系统日志」
3. 选择后端或前端日志，并选择最近 200、1000 或 5000 行的加载窗口
4. 使用精确日志级别和关键词筛选当前窗口
5. 复制日志前可以关闭「自动刷新」和「跟随最新日志」，页面不会再跳到末尾
6. 支持单条复制、自动换行和下载完整日志文件

日志页面每 2 秒只读取新增内容，不会重复下载整个日志文件。关闭「跟随最新日志」后，新日志仍会加载，但页面只显示新日志数量提示，点击提示才滚动到最新位置。

「清空日志」会截断当前日志内容但保留活动文件和写入句柄，清空后无需重启容器即可继续写入。
