...
2. 记下该配置的 Key，例如 `ostrm`。
3. 为配置内的通知目标设置标签；OStrm 默认投递到 `all`。

OStrm 默认调用地址为 `http://apprise:8000/notify/ostrm/`，其中服务地址和配置 Key 都可在系统设置中修改。
...
- **每类最多展示**：默认为 5，分别限制「刮削未识别」「目录重命名失败」「文件重命名失败」等分组的明细数量。

保存前可点击「发送测试通知」直接验证当前表单中的 Apprise 地址、Key 和标签。正式通知发送失败只会记录日志，不会改变任务或手动刮削的最终状态。
